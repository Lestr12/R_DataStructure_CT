...
```{r}
names <- c("Liz" ," Lizbeth", "Lenny", "larry", "love")
names
```
...
```{r}
my_vector <- c(12,35,32,-22,-12, 0, 1200)
sort(my_vector)
...
```{r}
times_vectors <- vector_1 * vector_2
times_vectors
```
...
student_names <- c("liz", "mary","lily", "Jose", "James", "Jenny")
...
```{r}
student_name <- c("liz", "mary","lily")
student_grade <- c("Sopomore" , "Junior", " Senior")
student_class <-c(65, 82, 89)

df <- data.frame(student_name, student_grade, student_class)
df
```
...
df$student_subject <- c( "Math", "Science", "Chemstry")
...
student_name <- c("Liz", "James")
student_grade_level <- c("Freshman", " Senior")
student_class_grade <-c( 82, 89)
student_subject <-c("Math", "Science")
...
my_array <- array(
  ...
  dim <- c(3,3,3),
  #row/column names
  dimnames <- list(c("Row 1", "Row 2", "Row 3"), c("Col 1", "Col 2", "LUNCH"),
                   c("MATRIX 1", "MATRIX RELOADED", "MATRIX 3"))
)
...
```{r}
first_matrix <- matrix(c(1,2,3,4,5,6,7,8,9),
                       nrow <- 3,
                       ncol <- 3,
                       byrow <- FALSE,
                       dimnames <- list(c("R1_1","R2_1","R3_1"), c("C1_1","C2_1","C3_1"))
                       )



Second_matrix <- matrix( c(9,8,7,6,5,4,3,2,1) ,
                       nrow <- 3,
                       ncol <- 3,
                       byrow <- FALSE,
                       dimnames <- list(c("R1_2","R2_2","R3_2"), c("C1_2","C2_2","C3_2"))
                       )
...
Array1 <- array(
  ...
  dim <- c(3,3,2),
  dimnames <- list(c("Row 1", "Row 2", "Row 3"), c("Col 1", "Col 2", "Col3"),
                   c("MATRIX 1", "MATRIX 2"))
)
Array1
```
